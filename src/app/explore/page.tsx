  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-pink-400 text-sm mt-1">Discover amazing food from the community</p>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full h-12 pl-11 pr-4 rounded-2xl border border-pink-200 bg-white/80 shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-pink-400 placeholder:text-pink-300"
        />
      </div>

      {/* User search results */}
      {query.trim() && (
        <div className="mb-6 bg-white/80 backdrop-blur-sm rounded-3xl border border-pink-100 shadow-sm overflow-hidden">
          {searching && (
            <p className="text-sm text-pink-400 text-center py-4">Searching...</p>
          )}
          {!searching && users.length === 0 && (
            <p className="text-sm text-pink-400 text-center py-4">No users found</p>
          )}
          {users.map((u) => (
            <Link
              key={u.id}
            ...
            >
              ...
              <div>
                ...
                {u.name && <p className="text-sm text-pink-400">{u.name}</p>}
              </div>
            </Link>
          ))}
        </div>
      ...
      {!query.trim() && (
        loading ? (
          <div className="grid grid-cols-3 gap-1">
            {[...Array(9)].map((_, i) => <div key={i} className="aspect-square bg-pink-100 rounded-xl animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-1">
            {posts.map((post, i) => (
