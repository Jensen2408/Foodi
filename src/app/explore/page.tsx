...
export default function ExplorePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {/* User search results */}
      {query.trim() && (
        <div className="mb-6 rounded-3xl border border-gray-200 overflow-hidden bg-white shadow-sm">
          {searching && (
          ...
          {users.map((u) => (
            ...
            >
              ...
              <div>
                ...
                {u.name && <p className="text-sm text-gray-400">{u.name}</p>}
              </div>
            </Link>
          ))}
        </div>
      ...
      {!query.trim() && (
        loading ? (
          <div className="grid grid-cols-3 gap-1">
            {[...Array(9)].map((_, i) => <div key={i} className="aspect-square rounded-xl animate-pulse" style={{background:"rgba(0,0,0,0.05)"}} />)}
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-1">
            {posts.map((post, i) => (
              ...
              >
                <div className="aspect-square relative">
                  {post.images[0] ? (
                    ...
                    <div className="w-full h-full" style={{background:"rgba(0,0,0,0.05)"}} />
                  )}
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100">
                    <span className="flex items-center gap-1 text-gray-900 text-sm font-bold">
