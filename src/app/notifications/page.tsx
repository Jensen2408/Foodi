  ...
  return (
    <div className="max-w-lg mx-auto px-0 pb-20 md:pb-0">
        ...
        <div className="divide-y divide-gray-100">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-center gap-3 px-4 py-3 animate-pulse">
              <div className="w-10 h-10 rounded-full shrink-0" style={{background:"rgba(0,0,0,0.05)"}} />
              ...
              <div className="w-10 h-10 rounded-lg shrink-0" style={{background:"rgba(0,0,0,0.05)"}} />
            </div>
          ))}
        </div>
      )}
      ...
      {!loading && items.length > 0 && (
        <div className="divide-y divide-gray-100">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-50">
              <div className="relative shrink-0">
                <Avatar src={item.user.avatar} alt={item.user.username} size="md" />
                <span className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full flex items-center justify-center bg-white border border-gray-200">
                  {icons[item.type]}
                </span>
              </div>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-700 leading-snug">
                  <Link href={`/profile/${item.user.username}`} className="font-bold text-gray-900 hover:text-purple-500 transition-colors">
                    {item.user.username}
                  </Link>{" "}
                ...
                <p className="text-xs text-gray-300 mt-0.5">{timeAgo(item.createdAt)}</p>
              </div>
