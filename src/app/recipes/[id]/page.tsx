  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {recipe.coverImage ? (
        ...
        <div className="aspect-video rounded-3xl bg-gradient-to-br from-brand-100 to-pink-100 flex flex-col items-center justify-center mb-6">
          <BookOpen className="w-16 h-16 text-brand-300 mb-3" />
          <h1 className="text-3xl font-black text-gray-800 text-center px-6">{recipe.title}</h1>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6 flex items-center justify-between">
        <Link href={`/profile/${recipe.user.username}`} className="flex items-center gap-3 group">
          ...
          <div>
            <p className="font-semibold text-gray-900 group-hover:text-brand-600 transition-colors">{recipe.user.username}</p>
            {recipe.user.name && <p className="text-sm text-gray-500">{recipe.user.name}</p>}
          </div>
        </Link>
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 justify-end">
            {tags.map((tag) => (
              <span key={tag} className="text-xs bg-brand-50 text-brand-700 px-2.5 py-1 rounded-full font-medium">{tag}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: "Cook", value: recipe.cookTime ? `${recipe.cookTime}m` : "—", icon: Clock, color: "text-[#c6185c]" },
        ...
        <ul className="space-y-2">
          {ingredients.map((ing, i) => (
            <li key={i} className="flex items-start gap-3 text-gray-700">
              <span className="w-6 h-6 bg-brand-50 text-brand-600 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{i + 1}</span>
              {ing}
            </li>
          ))}
        </ul>
