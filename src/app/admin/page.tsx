...
export default function AdminPage() {
  ...
  const statCards = stats ? [
    { label: "Users", value: stats.users, icon: Users, color: "bg-pink-500" },
    ...
    { label: "Comments", value: stats.comments, icon: MessageCircle, color: "bg-pink-400" },
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-pink-400 mt-1">Manage FoodGram</p>
        </div>
        <span className="flex items-center gap-2 bg-pink-100 text-pink-700 font-semibold text-sm px-4 py-2 rounded-full">
          <Shield className="w-4 h-4" /> Admin
        </span>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 mb-6 bg-pink-50 p-1 rounded-2xl w-fit">
        {([["stats", "Stats", BarChart3], ["users", "Users", Users], ["posts", "Posts", ImageIcon]] as const).map(([key, label, Icon]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === key ? "bg-white shadow text-pink-700" : "text-pink-400 hover:text-pink-600"}`}
          >
      ...
      {tab === "stats" && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {statCards.map(({ label, value, icon: Icon, color }) => (
            <div key={label} className="bg-white/80 backdrop-blur-sm rounded-3xl border border-pink-100 shadow-sm p-6 flex items-center gap-4">
      ...
      {/* Users tab */}
      {tab === "users" && (
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-pink-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-pink-50">
            <p className="font-bold text-pink-900">{users.length} users</p>
          </div>
          <div className="divide-y divide-pink-50">
            {users.map(u => (
              <div key={u.id} className="flex items-center gap-4 px-6 py-4 hover:bg-pink-50/50 transition-colors">
                <Avatar src={u.avatar} alt={u.username} size="md" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Link href={`/profile/${u.username}`} className="font-semibold text-gray-900 hover:text-pink-600">@{u.username}</Link>
                    {u.isAdmin && <span className="text-xs bg-pink-100 text-pink-600 px-2 py-0.5 rounded-full font-semibold">Admin</span>}
                  </div>
                  <p className="text-sm text-pink-400 truncate">{u.email}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={`p-2 rounded-xl transition-colors ${u.isAdmin ? "text-pink-500 hover:bg-pink-100" : "text-gray-400 hover:bg-gray-100"}`}
                  >
      ...
      {tab === "posts" && (
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-pink-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-pink-50">
            {posts.map(p => (
              <div key={p.id} className="flex items-center gap-4 px-6 py-4 hover:bg-pink-50/50 transition-colors">
                {p.images[0] ? (
                  <div className="w-14 h-14 rounded-2xl overflow-hidden shrink-0">
                    <Image src={p.images[0].url} alt="" width={56} height={56} className="object-cover w-full h-full" />
                  </div>
                ) : (
                  <div className="w-14 h-14 rounded-2xl bg-pink-100 shrink-0" />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Link href={`/profile/${p.user.username}`} className="text-sm font-semibold text-pink-600">@{p.user.username}</Link>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link href={`/post/${p.id}`} className="p-2 rounded-xl text-gray-400 hover:text-pink-500 hover:bg-pink-50 transition-colors text-xs font-medium">
                    View
                  </Link>
