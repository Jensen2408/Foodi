    ...
    <article ref={cardRef} onMouseMove={handleTilt} onMouseLeave={resetTilt} style={{transition:"transform 0.15s ease", willChange:"transform"}} className="rounded-2xl overflow-hidden border border-gray-200 bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <Link href={`/profile/${post.user.username}`} className="flex items-center gap-3 group">
          ...
          <div>
            <p className="text-sm font-bold text-gray-900 group-hover:text-purple-500 transition-colors leading-tight">
              {post.user.name || post.user.username}
            </p>
            {post.location ? (
              <p className="text-xs text-pink-500 flex items-center gap-0.5 leading-tight">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-10 w-52 rounded-2xl border border-gray-200 overflow-hidden z-50 animate-slide-up bg-white shadow-lg">
                {isOwner ? (
                  <>
                    <button
                    ...
                    <Link
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50 text-gray-500 transition-colors text-sm"
                    >
      ...
      <div className="px-4 pt-3 pb-4 space-y-2.5">
        ...
        {post.caption && (
          <p className="text-sm text-gray-600 leading-snug">
            <Link href={`/profile/${post.user.username}`} className="font-bold text-gray-900 mr-1 hover:text-purple-500">
              {post.user.name || post.user.username}
            </Link>
        ...
        {comments.map((c) => (
          <p key={c.id} className="text-sm">
            <span className="font-bold text-gray-900 mr-1">{c.user.username}</span>
            <span className="text-gray-600">{c.text}</span>
          </p>
        ))}

        <p className="text-xs text-gray-300 uppercase tracking-wide">{formatRelativeTime(post.createdAt)}</p>

        {showRecipe && recipe && (
          <div className="mt-1 p-4 rounded-2xl border border-purple-200 space-y-3 bg-purple-50">
            <div className="flex items-center gap-2">
